...
def build_readme():
    ...
    shap = json.loads((ROOT / "results/shap_metadata.json").read_text())
    holdout = results["holdout"]
    rows = []
    for name, entry in results["cross_validation"].items():
        rows.append("| " + name.replace("_", " ") + " | " + " | ".join(
            f"{entry['metrics'][metric]['mean']:.4f} ± {entry['metrics'][metric]['std']:.4f}"
            for metric in ["roc_auc", "accuracy", "precision", "recall", "f1"]) + " |")
    cv_table = "\n".join(rows)
    holdout_table = "\n".join(f"| {name.replace('_', ' ').title()} | {holdout[name]:.6f} |" for name in
        ["accuracy", "precision", "recall", "f1", "specificity", "roc_auc", "average_precision"])
...
Selected model: **{results['selected_model'].replace('_', ' ')}**.
